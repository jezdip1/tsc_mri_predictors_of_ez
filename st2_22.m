...
for ii=1:size(ids,1)
 i=1;
for qqi=find(undepend_variables.test==1)'
    PP=ids(ii);
    [RHOkend(i,ii),PVALkend(i,ii)] = corr([T.Resectedarea(T.PtNo==PP)],T{T.PtNo==PP,qqi},'type','Kendall');
    XTENT.resectionExtent(ii)=sum(T.Resectedarea(T.PtNo==PP));
    XTENT{ii,i+1}=max(T{T.PtNo==PP,qqi});
...
save ('./XTENT2.mat','XTENT')
figure (5)
[R P]=corr(table2array(XTENT),'rows','pairwise','type','spearman');
imagesc(R)
xticklabels(XTENT.Properties.VariableNames)
yticklabels(XTENT.Properties.VariableNames)
set(gca,'XTick',[1:size(XTENT,2)])
set(gca,'YTick',[1:size(XTENT,2)])
set(gca,'XTickLabelRotation',45)
% title('Cross Correlation matrix')
title('Cross Correlation matrix reviewer 1')
